...
def doScreenshot(additionalArguments):
    # file = os.path.join(getScreenshotDirectory(), "%Y%m%d_%T.png")
    file = os.path.join(getScreenshotDirectory(), "%Y%m%d_%H%M%S.png")

    # command = "sleep 0.1 && scrot --exec 'xclip -selection c -t image/png < $f' %s " % file
    command = "sleep 0.1 && scrot --exec 'echo $f | xclip -selection c' %s " % file
